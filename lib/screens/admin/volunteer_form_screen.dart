...
class VolunteerFormScreen extends StatefulWidget {
  ...
  const VolunteerFormScreen({super.key, this.opportunity});
...
class _VolunteerFormScreenState extends State<VolunteerFormScreen> {
  ...
  late TextEditingController _maxVolunteersCtrl;
  late TextEditingController _imageUrlCtrl;

  final List<String> _categories = [
    'Community', 'Charity', 'Education', 'Mosque', 'Youth', 'Other'
  ];
  ...
  void initState() {
    ...
    _maxVolunteersCtrl = TextEditingController(
      text: (o?.maxVolunteers ?? 50).toString(),
    );
    _imageUrlCtrl = TextEditingController(text: o?.imageUrl ?? '');
    _previewUrl = o?.imageUrl ?? '';
    final existing = o?.category ?? '';
    _category = _categories.contains(existing) ? existing : _categories.first;
    _isActive = o?.isActive ?? true;
    if (o != null) {
      _date = o.dateTime;
      _time = TimeOfDay.fromDateTime(o.dateTime);
    }
  }
  ...
  Future<void> _save() async {
    ...
    final data = {
      ...
      'currentVolunteers': widget.opportunity?.currentVolunteers ?? 0,
      'isActive': _isActive,
      'imageUrl': _imageUrlCtrl.text.trim(),
      'organizerId': widget.opportunity?.organizerId ?? '',
    };

    try {
      if (_isEditing) {
        await _adminService.updateVolunteerOpportunity(
            widget.opportunity!.opportunityId, data);
      } else {
        await _adminService.createVolunteerOpportunity(data);
      }
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: Text(_isEditing ? 'Opportunity updated!' : 'Opportunity created!'),
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text(
          _isEditing ? 'Edit Opportunity' : 'New Opportunity',
        ...
        child: ListView(
          ...
          children: [
            ...
            TextFormField(
              ...
              decoration: _decoration('Opportunity title'),
            ...
            TextFormField(
              ...
              decoration: _decoration('Describe the volunteer opportunity'),
              maxLines: 4,
              textCapitalization: TextCapitalization.sentences,
              validator: (v) => (v == null || v.trim().isEmpty) ? 'Required' : null,
            ),
            ...
            _label('Requirements (optional)'),
            TextFormField(
              controller: _requirementsCtrl,
              decoration: _decoration('e.g. Age 18+, physical fitness required'),
              maxLines: 3,
              textCapitalization: TextCapitalization.sentences,
            ),
            const SizedBox(height: 20),
            ...
            SizedBox(
              ...
              child: ElevatedButton(
                ...
                child: Text(
                  _isEditing ? 'Update Opportunity' : 'Create Opportunity',
  ...
  Widget _buildImageSection() {
    return Column(
      ...
      children: [
        ...
        Text(
          'Tip: unsplash.com → right-click image → Copy image address',
  ...
  Widget _imagePlaceholder() => Column(
        ...
        children: [
          ...
          Text('No image added', style: TextStyle(color: Colors.grey[400], fontSize: 13)),
        ],
      );
